# ================================================================
# Exploración visual de MFCCs (Fase 1 Benchmark por Sitio)
# Autor: Alcides Rojas
# Objetivo: Explorar features_NOMBRE DE LA CARPETA.parquet con visualizaciones claras
# ================================================================
...
# Sitios a mostrar (reemplazar por el sitio)
SITIOS_SELECCIONADOS = ["CANTERA1Tapy"]
...
def guardar_fig(nombre: str):
    os.makedirs("figuras_CANTERA1Tapy", exist_ok=True)
    plt.savefig(os.path.join("figuras_CANTERA1Tapy", f"{nombre}.png"), bbox_inches="tight")
...
if "mfcc_mean_1" in cols_mfcc_mean:
    ...
    plt.title("Distribución de mfcc_mean_1 (CANTERA1Tapy)")
...
if "mfcc_mean_1" in cols_mfcc_mean and "sitio" in df.columns:
    ...
    plt.title("mfcc_mean_1 por sitio (CANTERA1Tapy)")
    plt.xlabel("Sitio")
...
if len(cols_corr_5) >= 2:
    ...
    plt.title("Correlación entre mfcc_mean_1..5 (CANTERA1Tapy)")
...
if "mfcc_mean_1" in cols_mfcc_mean and "mfcc_sd_1" in cols_mfcc_sd:
    ...
    plt.suptitle("Distribución de medias y desviaciones (CANTERA1Tapy)", y=1.02, fontsize=12, fontweight="bold")
...
if len(cols_corr_10) >= 3:
    ...
    plt.title("Correlación entre mfcc_mean_1..10 (CANTERA1Tapy)")
...
if "mfcc_mean_1" in df.columns and "sitio" in df.columns:
    ...
    plt.title("Promedio de mfcc_mean_1 por sitio (CANTERA1Tapy)")
