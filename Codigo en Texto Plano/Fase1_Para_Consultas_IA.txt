# ================================================================
# Fase 1 Benchmark (multi-sitio):
# Extracción de MFCCs con filtro horario + atenuación 6–12 kHz
# Sitios: BO-81Tapyta, BO-82Tapyta, CANTERA1Tapy, EU-41Tapyta, PA-41Tapyta
# ================================================================
...
def correr_fase1_para_sitio(sitio: str):
    ...
    ruta_temporales = ruta_salida / f"temporales_fase1_{sitio}_{SUFIJO_CORRIDA}"
    ruta_temporales.mkdir(parents=True, exist_ok=True)

    ruta_features_final = ruta_salida / f"features_{sitio}_{SUFIJO_CORRIDA}.parquet"
    ruta_resumen = ruta_salida / f"resumen_segmentos_por_sitio_{sitio}_{SUFIJO_CORRIDA}.csv"

    carpeta = ruta_base_externa / sitio
    data_dir = carpeta / "Data"
    if not data_dir.exists():
        print(f"⚠️ No se encontró la carpeta 'Data' en {carpeta}, se salta este sitio.")
        return
    ...
    lotes = sorted((ruta_salida / f"temporales_fase1_{sitio}_{SUFIJO_CORRIDA}").glob("lote_*.parquet"))
    if len(lotes) == 0:
